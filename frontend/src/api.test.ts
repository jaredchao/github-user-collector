...
describe("fetchUser", () => {
  it("returns the saved profile on 201", async () => {
...
describe("fetchIntro", () => {
  ...
  // Generation is asynchronous, so an empty or missing introduction just
  // means the worker hasn't finished yet.
  it("keeps polling until the worker has generated the introduction", async () => {
    const spy = vi
      .fn()
      .mockResolvedValueOnce({ ok: true, status: 200, json: async () => ({ intro: "" }) })
      .mockResolvedValueOnce({ ok: false, status: 404, json: async () => ({}) })
      .mockResolvedValue({ ok: true, status: 200, json: async () => ({ intro: "done" }) });
    vi.stubGlobal("fetch", spy);

    await expect(fetchIntro("torvalds", { intervalMs: 1, timeoutMs: 500 })).resolves.toBe("done");
    expect(spy).toHaveBeenCalledTimes(3);
  });

  it("gives up with a readable message when generation never finishes", async () => {
    const spy = vi.fn().mockResolvedValue({ ok: true, status: 200, json: async () => ({}) });
    vi.stubGlobal("fetch", spy);

    await expect(fetchIntro("torvalds", { intervalMs: 1, timeoutMs: 20 })).rejects.toThrow(
      "介绍生成超时",
    );
  });

  it("reports a timeout rather than 404 when the profile never gets one", async () => {
    mockResponse(404);
    await expect(fetchIntro("nobody", { intervalMs: 1, timeoutMs: 20 })).rejects.toThrow(
      "介绍生成超时",
    );
  });
  ...
  it("maps 503 (Go service down) onto a readable message", async () => {
    mockResponse(503);
    await expect(fetchIntro("torvalds", { intervalMs: 1, timeoutMs: 500 })).rejects.toThrow(
      "介绍服务暂时不可用",
    );
  });
